...
class AttendanceLogger:
    ...
    def _create_new_workbook(self):
        """Create the master workbook with styled headers (extended schema)."""
        self.wb = Workbook()
        self.ws = self.wb.active
        ...
        headers    = ["S.No", "Roll No", "Enrollment No", "Student Name",
                      "Date", "Time", "Status", "Status Code"]
        col_widths = [8,       15,        18,              25,
                      15,      12,        12,              12]
    ...
    def mark_present(self, name: str, roll_no: str,
                     enrollment_no: str = "", status_code: str = "P") -> bool:
        """
        ...
        enrollment_no : Optional enrollment number (new field, defaults to '')
        status_code   : Attendance code — P / OD / ML (defaults to 'P')
    ...
    def get_present_list(self) -> list:
        ...
        Returns
        -------
        list of dicts: {name, roll_no, enrollment_no, status_code, time}
        """
    ...
    def _flush_to_sheet(self):
        ...
        Writes extended columns: S.No | Roll No | Enrollment No | Student Name |
                                 Date | Time | Status | Status Code
        """
        ...
        # Determine column offset — detect whether the sheet uses old (6-col) or new (8-col) schema
        # by checking the header row. This ensures backward-compatibility with existing files.
        header_row = [c.value for c in self.ws[1]]
        is_extended = len(header_row) >= 8 and header_row[2] == "Enrollment No"

        if is_extended:
            # Remove existing rows for those dates to prevent duplicates (col D = Date = index 4)
            rows_to_delete = [
                row[0].row for row in self.ws.iter_rows(min_row=2)
                if len(row) >= 5 and row[4].value and str(row[4].value).strip() in cache_dates
            ]
        else:
            rows_to_delete = [
                row[0].row for row in self.ws.iter_rows(min_row=2)
        ...
        for entry in self._present_cache:
            ...
            enroll_no   = entry.get("enrollment_no", "")
            status_code = entry.get("status_code", "P")
