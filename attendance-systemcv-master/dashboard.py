...
# ── Optional academic modules (graceful degradation if not yet installed) ────
try:
    from attendance_manager import AttendanceSession, ATTENDANCE_CODES, load_course_history
    from academic_db import (
        get_programs, get_semesters, get_sections, get_courses,
        get_students, import_students_from_excel, add_student,
...
# ═══════════════════════════════════════════════════════════════════════════════
...
elif page == "📚 Academic Attendance":
    st.markdown(
        "<h1 style='color:#fff;margin-bottom:0'>📚 Academic Attendance Management</h1>",
        unsafe_allow_html=True,
    )
    st.markdown(
        "<p style='color:#8888aa'>Manage attendance by Program → Semester → Section → Course.</p>",
        unsafe_allow_html=True,
    )

    if not _ACADEMIC_MODULES_AVAILABLE:
        st.error(
            "⚠️ The `academic_db` and `attendance_manager` modules are not installed or could not be "
            "imported. Please ensure both files (`academic_db.py` and `attendance_manager.py`) are "
            "present in the project directory and all their dependencies are installed, then restart "
            "the dashboard."
        )
        st.stop()

    # ── Section A: Academic Selector ─────────────────────────────────────────
    st.markdown("<div class='section-title'>🎓 Academic Selector</div>", unsafe_allow_html=True)
    ...
    try:
        ...
        st.error(f"Failed to load programs: {_e}")
        programs_list = []

    with sel_col1:
        acad_program = st.selectbox(
            "Program",
            programs_list if programs_list else ["(No programs)"],
            key="acad_program",
        )

    try:
        semesters_list = get_semesters(acad_program) if programs_list else []
    except Exception as _e:
        st.error(f"Failed to load semesters: {_e}")
        semesters_list = []

    with sel_col2:
        acad_semester = st.selectbox(
            "Semester",
            semesters_list if semesters_list else ["(No semesters)"],
            key="acad_semester",
        )

    try:
        sections_list = get_sections(acad_program, acad_semester) if semesters_list else []
    except Exception as _e:
        st.error(f"Failed to load sections: {_e}")
        sections_list = []

    with sel_col3:
        acad_section = st.selectbox(
            "Section",
            sections_list if sections_list else ["(No sections)"],
            key="acad_section",
        )

    try:
        courses_list = get_courses(acad_program, acad_semester) if semesters_list else []
    except Exception as _e:
        st.error(f"Failed to load courses: {_e}")
        courses_list = []

    with sel_col4:
        acad_course = st.selectbox(
            "Course",
            courses_list if courses_list else ["(No courses)"],
            key="acad_course",
        )

    # Detect if the selection changed so we can reset marks
    _selector_key = f"{acad_program}|{acad_semester}|{acad_section}|{acad_course}"
    if st.session_state.get("_acad_selector_key") != _selector_key:
        st.session_state["_acad_selector_key"] = _selector_key
        st.session_state["acad_loaded"] = False
        st.session_state["acad_marks"] = {}
        st.session_state["acad_students"] = []

    load_btn = st.button("📂 Load Students", key="acad_load_btn", type="primary")
    if load_btn:
        try:
            _students = get_students(acad_program, acad_semester, acad_section)
            st.session_state["acad_students"] = _students
            # Initialise every student to 'A' if not already present
            existing_marks = st.session_state.get("acad_marks", {})
            fresh_marks = {}
            for _s in _students:
                _roll = str(_s.get("roll_no", _s.get("roll", "")))
                fresh_marks[_roll] = existing_marks.get(_roll, "A")
            st.session_state["acad_marks"] = fresh_marks
            st.session_state["acad_loaded"] = True
        except Exception as _e:
            st.error(f"Failed to load students: {_e}")
            st.session_state["acad_loaded"] = False

    # ── Section B: Student List & Attendance Entry ───────────────────────────
    if st.session_state.get("acad_loaded") and st.session_state.get("acad_students"):
        acad_students = st.session_state["acad_students"]
        acad_marks    = st.session_state.get("acad_marks", {})

        st.markdown("<br>", unsafe_allow_html=True)
        st.markdown(
            f"<div class='section-title'>📋 Attendance Sheet "
            f"<span style='color:#8888aa;font-size:0.85rem'>({len(acad_students)} students)</span></div>",
            unsafe_allow_html=True,
        )
        st.caption(f"Program: {acad_program} | Semester: {acad_semester} | Section: {acad_section} | Course: {acad_course}")

        # Build a DataFrame for the data editor
        _status_options = ["P", "A", "OD", "ML"]
        _editor_rows = []
        for _s in acad_students:
            _roll = str(_s.get("roll_no", _s.get("roll", "")))
            _enroll = str(_s.get("enrollment_no", _s.get("enrollment", "")))
            _name  = str(_s.get("name", _s.get("student_name", "")))
            _status = acad_marks.get(_roll, "A")
            _editor_rows.append({
                "Roll No":       _roll,
                "Enrollment No": _enroll,
                "Name":          _name,
                "Status":        _status,
            })
        ...
        _edited_df = st.data_editor(
            ...
            column_config={
                ...
                "Enrollment No": st.column_config.TextColumn("Enrollment No", disabled=True),
                "Name":          st.column_config.TextColumn("Name",          disabled=True),
                "Status":        st.column_config.SelectboxColumn(
                    "Status",
                    options=_status_options,
                    required=True,
                    help="P = Present, A = Absent, OD = On Duty, ML = Medical Leave",
                ),
            },
        ...
        # Sync edited values back to session_state marks
        if _edited_df is not None:
            for _, _row in _edited_df.iterrows():
                _r = str(_row["Roll No"])
                acad_marks[_r] = str(_row["Status"])
            st.session_state["acad_marks"] = acad_marks

        # ── Section C: Live Statistics ────────────────────────────────────────
        st.markdown("<br>", unsafe_allow_html=True)
        st.markdown("<div class='section-title'>📊 Live Statistics</div>", unsafe_allow_html=True)

        _total  = len(acad_marks)
        _cnt_p  = sum(1 for v in acad_marks.values() if v == "P")
        _cnt_a  = sum(1 for v in acad_marks.values() if v == "A")
        _cnt_od = sum(1 for v in acad_marks.values() if v == "OD")
        _cnt_ml = sum(1 for v in acad_marks.values() if v == "ML")
        _att_pct = round((_cnt_p + _cnt_od) / _total * 100, 1) if _total > 0 else 0.0

        _mc1, _mc2, _mc3, _mc4, _mc5 = st.columns(5)
        with _mc1:
            st.metric("👥 Total", _total)
        with _mc2:
            st.metric("✅ Present", _cnt_p)
        with _mc3:
            st.metric("❌ Absent", _cnt_a)
        with _mc4:
            st.metric("🔄 OD", _cnt_od)
        with _mc5:
            st.metric("🏥 ML", _cnt_ml)

        # Colour-coded progress bar
        _bar_color = "#00C853" if _att_pct >= 75 else ("#FF6D00" if _att_pct >= 50 else "#FF1744")
        st.markdown(
            f"**Effective Attendance (P + OD): {_att_pct}%**",
        )
        st.markdown(
            f"""<div style="background:rgba(255,255,255,0.08);border-radius:8px;overflow:hidden;height:14px;margin-bottom:1rem;">
              <div style="width:{_att_pct}%;height:14px;background:{_bar_color};border-radius:8px;transition:width 0.4s;"></div>
            </div>""",
            unsafe_allow_html=True,
        )
        ...
        st.markdown("<div class='section-title'>📤 Import Attendance from Excel</div>", unsafe_allow_html=True)
        _uploaded = st.file_uploader(
            "Upload attendance sheet (.xlsx)",
            type=["xlsx"],
            key="acad_import_uploader",
            help="Upload a previously exported attendance sheet to merge/overwrite marks.",
        )
        if _uploaded is not None:
            try:
                _import_bytes = _uploaded.read()
                _tmp_session  = AttendanceSession(
                    program=acad_program,
                    semester=acad_semester,
                    section=acad_section,
                    course=acad_course,
                )
                _merged_count = _tmp_session.import_from_bytes(_import_bytes)
                # Merge imported marks into current session state
                _imported_marks = _tmp_session.get_marks()
                _before = dict(acad_marks)
                acad_marks.update(_imported_marks)
                st.session_state["acad_marks"] = acad_marks
                st.success(
                    f"✅ Imported {_merged_count} mark(s) from **{_uploaded.name}**. "
                    f"Session state updated — scroll up to review."
                )
            except Exception as _e:
        ...
        # ── Section E: Export Excel ────────────────────────────────────────────
        st.markdown("<div class='section-title'>📥 Export Attendance Sheet</div>", unsafe_allow_html=True)

        if st.button("📊 Generate Export", key="acad_export_btn"):
            try:
                _export_session = AttendanceSession(
                    program=acad_program,
                    semester=acad_semester,
                    section=acad_section,
                    course=acad_course,
                ...
                _export_bytes = _export_session.export_to_bytes()
                _export_filename = (
                    f"attendance_{acad_program}_{acad_semester}_{acad_section}"
                    f"_{acad_course}_{date.today()}.xlsx"
                )
                st.download_button(
                    label="⬇️ Download Attendance Sheet (.xlsx)",
                    data=_export_bytes,
                    file_name=_export_filename,
                    mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
                    use_container_width=True,
                )
                st.success(f"✅ Export ready — click the button above to download **{_export_filename}**.")
            except Exception as _e:
                st.error(f"Export failed: {_e}")

        st.divider()

        # ── Section F: Student Management ─────────────────────────────────────
        with st.expander("➕ Add New Student to Section"):
            _add_c1, _add_c2, _add_c3 = st.columns(3)
            with _add_c1:
                _new_roll   = st.text_input("Roll No",       key="acad_new_roll",   placeholder="e.g. 22CS001")
            with _add_c2:
                _new_enroll = st.text_input("Enrollment No", key="acad_new_enroll", placeholder="e.g. 0901CS221001")
            with _add_c3:
                _new_name   = st.text_input("Student Name",  key="acad_new_name",   placeholder="Full name")

            if st.button("✅ Add Student", key="acad_add_btn"):
                if not _new_roll.strip() or not _new_name.strip():
                    st.warning("Roll No and Name are required.")
                else:
                    try:
                        add_student(
                            program=acad_program,
                            semester=acad_semester,
                            section=acad_section,
                            roll_no=_new_roll.strip(),
                            enrollment_no=_new_enroll.strip(),
                            name=_new_name.strip(),
                        )
                        st.success(
                            f"✅ Student **{_new_name.strip()}** (Roll: {_new_roll.strip()}) "
                            f"added to {acad_section}. Click **Load Students** again to refresh the list."
                        )
                    except Exception as _e:
                        st.error(f"Failed to add student: {_e}")

    elif st.session_state.get("acad_loaded") and not st.session_state.get("acad_students"):
        st.info("No students found for the selected Program / Semester / Section. Use **Add New Student** below after loading.")

    else:
        st.info("👆 Select a Program, Semester, Section, and Course above, then click **Load Students**.")
...
# ═══════════════════════════════════════════════════════════════════════════════
...
elif page == "🎪 Event Attendance":
    st.markdown("<h1 style='color:#fff;margin-bottom:0'>🎪 Event & Activity Attendance</h1>", unsafe_allow_html=True)
    st.markdown(
        "<p style='color:#8888aa'>Record approved event/activity participation and automatically link "
        "activity engagement to student attendance records.</p>",
        unsafe_allow_html=True,
    )

    if not _ACADEMIC_MODULES_AVAILABLE:
        st.error("Academic & Event modules are not available. Ensure `academic_db.py` is present.")
        st.stop()

    # ── Step 1: Select or Add Event ──────────────────────────────────────────
    st.markdown("<div class='section-title'>🎯 Step 1 — Select Approved Event / Activity</div>", unsafe_allow_html=True)

    try:
        raw_events = get_events()
        event_names = [e["name"] for e in raw_events]
    except Exception:
        event_names = ["Code Champ Competition", "Tech Hackathon", "Annual Sports Meet", "Cultural Fest"]

    col_evt1, col_evt2 = st.columns([2.5, 1.5])
    with col_evt1:
        sel_event = st.selectbox(
            "Select Event / Competition",
            event_names,
            key="dashboard_sel_event",
            help="Choose the approved competition or activity for which attendance is being awarded."
        )

    with col_evt2:
        with st.expander("➕ Add New Event"):
            new_evt_name = st.text_input("Event Name", key="dash_new_evt_name")
            new_evt_type = st.selectbox("Category", ["Technical", "Sports", "Cultural", "Academic", "Workshop"], key="dash_new_evt_type")
            if st.button("Save Event", key="dash_save_evt_btn"):
                if new_evt_name.strip():
                    add_event(new_evt_name.strip(), new_evt_type)
                    st.success(f"Event '{new_evt_name.strip()}' registered!")
                    st.rerun()

    # ── Step 2: Student Selection & Filtering ─────────────────────────────────
    st.markdown("<div class='section-title'>👥 Step 2 — Select Class / Students</div>", unsafe_allow_html=True)

    col_p1, col_p2, col_p3 = st.columns(3)
    try:
        prog_list = get_programs()
    except Exception:
        prog_list = ["BCA", "MCA", "B.Tech CSE"]

    with col_p1:
        evt_prog = st.selectbox("Program", prog_list, key="evt_prog_sel")
    with col_p2:
        sem_list = get_semesters(evt_prog) if prog_list else []
        evt_sem = st.selectbox("Semester", sem_list if sem_list else ["Semester 1"], key="evt_sem_sel")
    with col_p3:
        sec_list = get_sections(evt_prog, evt_sem) if sem_list else ["A"]
        evt_sec = st.selectbox("Section", sec_list if sec_list else ["A"], key="evt_sec_sel")

    # Fetch students for this class
    class_students = get_students(evt_prog, evt_sem, evt_sec)
    if not class_students:
        # Fallback to enrolled face students
        class_students = [{"roll_no": s["roll_no"], "enrollment_no": "", "name": s["name"]} for s in enrolled]

    # Fetch existing recorded attendees for this event
    existing_records = get_event_attendance(sel_event)
    attended_rolls = {str(r.get("roll_no", "")).strip().lower() for r in existing_records}
    ...
    with kpi_c1:
        ...
        <div class="kpi-card">
          <div class="kpi-icon">🎪</div>
          <div class="kpi-value" style="font-size:1.3rem;color:#FF6D00">{sel_event}</div>
    ...
    with kpi_c2:
        ...
        <div class="kpi-card">
          <div class="kpi-icon">🎓</div>
          <div class="kpi-value">{len(class_students)}</div>
          <div class="kpi-label">Class Roster Size</div>
        </div>""", unsafe_allow_html=True)
    with kpi_c3:
        st.markdown(f"""
        <div class="kpi-card">
          <div class="kpi-icon">✅</div>
          <div class="kpi-value" style="color:#00C853">{len(existing_records)}</div>
          <div class="kpi-label">Total Event Attendees</div>
        </div>""", unsafe_allow_html=True)

    st.markdown("<br>", unsafe_allow_html=True)

    # ── Step 3: Fast Automatic Event Attendance Marking ───────────────────────
    st.markdown("<div class='section-title'>⚡ Step 3 — Record Event Attendance</div>", unsafe_allow_html=True)

    if not class_students:
        st.warning("No students found in this class roster. You can add students in Academic Attendance mode.")
    else:
        st.markdown(
            "Select students to mark for this event, or click **Mark Event** for instant individual logging. "
            "Participation is immediately linked to the **Activity Engagement** column."
        )

        # Bulk selection
        col_mark_all, col_spacer = st.columns([2, 3])
        with col_mark_all:
            if st.button("⚡ Mark All Displayed Students for This Event", key="evt_mark_all_btn", type="primary"):
                count_marked = 0
                for s in class_students:
                    record_event_attendance(sel_event, s["roll_no"], s["name"])
                    count_marked += 1
                st.success(f"✅ Automatically recorded event attendance for all {count_marked} students in '{sel_event}'!")
                st.rerun()
        ...
        for s in class_students:
            r_roll = s["roll_no"]
            r_name = s["name"]
            r_enrl = s.get("enrollment_no", "")
            is_done = r_roll.lower() in attended_rolls

            r_col1, r_col2, r_col3, r_col4 = st.columns([1.5, 2.5, 2, 1.8])
            with r_col1:
                st.markdown(f"**{r_roll}**")
            with r_col2:
                st.markdown(f"{r_name}")
            with r_col3:
                st.markdown(f"<small style='color:#8888aa'>{r_enrl if r_enrl else '(No Enrl No)'}</small>", unsafe_allow_html=True)
            with r_col4:
                if is_done:
                    st.markdown("<span style='color:#00C853;font-weight:bold'>✅ Attended (OD)</span>", unsafe_allow_html=True)
                else:
                    if st.button(f"🎪 Mark for Event", key=f"btn_evt_{r_roll}"):
                        record_event_attendance(sel_event, r_roll, r_name)
                        st.success(f"Recorded '{sel_event}' attendance for {r_name}!")
                        st.rerun()

    # ── Step 4: Event Participation Report & Export ───────────────────────────
    st.divider()
    st.markdown(f"<div class='section-title'>📋 Event Attendees Registry — {sel_event}</div>", unsafe_allow_html=True)

    if existing_records:
        rec_df = pd.DataFrame(existing_records)
        display_cols = [c for c in ["roll_no", "student_name", "date", "time", "status", "engagement", "remarks"] if c in rec_df.columns]
        st.dataframe(
        ...
        with col_ex1:
            ...
            st.download_button(
                "⬇️ Download Event Attendees CSV",
                data=csv_data,
                file_name=f"event_attendance_{sel_event.replace(' ', '_')}_{date.today()}.csv",
                mime="text/csv",
                use_container_width=True
            )
        with col_ex2:
            ...
            with pd.ExcelWriter(buf, engine="openpyxl") as writer:
                rec_df.to_excel(writer, index=False, sheet_name="Event Attendance")
            buf.seek(0)
            st.download_button(
                "⬇️ Download Event Attendees Excel (.xlsx)",
                data=buf,
                file_name=f"event_attendance_{sel_event.replace(' ', '_')}_{date.today()}.xlsx",
                mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
                use_container_width=True
            )
    else:
        st.info(f"No students have been recorded for '{sel_event}' yet. Use the buttons above to mark attendance.")
...
# ═══════════════════════════════════════════════════════════════════════════════
    ...
    if df_all.empty:
        ...
        # ── Competition Format Export ──────────────────────────────────────────
        st.divider()
        st.markdown(
            "<div class='section-title'>📋 Competition Format (.xlsx)</div>",
            unsafe_allow_html=True,
        )
        st.markdown(
            "Exports a structured attendance sheet compatible with the Academic Attendance format, "
            "mapping **Present** face-recognition records to **P** and absent students to **A**. "
            "Requires `attendance_manager` to be installed."
        )

        if _ACADEMIC_MODULES_AVAILABLE:
            _comp_col1, _comp_col2, _comp_col3 = st.columns(3)
            with _comp_col1:
                _comp_program  = st.text_input("Program",  value="GENERAL", key="comp_program")
            with _comp_col2:
                _comp_semester = st.text_input("Semester", value="1",       key="comp_semester")
            with _comp_col3:
                _comp_section  = st.text_input("Section",  value="A",       key="comp_section")
            _comp_course = st.text_input("Course / Subject", value="Attendance", key="comp_course")

            if st.button("📋 Generate Competition Format Export", key="comp_export_btn"):
                try:
                    # Build marks dict from export_df: each enrolled student gets P if they
                    # appear in at least one record in the range, A otherwise.
                    _comp_marks: dict[str, str] = {}
                    _present_rolls = set(
                        export_df["Roll No"].astype(str).str.strip().unique()
                    )
                    for _enr in enrolled:
                        _r = str(_enr.get("roll_no", "")).strip()
                        _comp_marks[_r] = "P" if _r in _present_rolls else "A"

                    # Students not in enrolled but in export_df are also captured
                    for _roll_raw in _present_rolls:
                        if _roll_raw not in _comp_marks:
                            _comp_marks[_roll_raw] = "P"

                    _comp_session = AttendanceSession(
                        program=_comp_program.strip() or "GENERAL",
                        semester=_comp_semester.strip() or "1",
                        section=_comp_section.strip() or "A",
                        course=_comp_course.strip() or "Attendance",
                        marks=_comp_marks,
                    )
                    _comp_bytes    = _comp_session.export_to_bytes()
                    _comp_filename = (
                        f"competition_{_comp_program}_{_comp_semester}_{_comp_section}"
                        f"_{_comp_course}_{date.today()}.xlsx"
                    )
                    st.download_button(
                        label="⬇️ Download Competition Format (.xlsx)",
                        data=_comp_bytes,
                        file_name=_comp_filename,
                        mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
                        use_container_width=True,
                    )
                    st.success(
                        f"✅ Competition export ready with **{len(_comp_marks)}** student records "
                        f"({sum(1 for v in _comp_marks.values() if v == 'P')} Present, "
                        f"{sum(1 for v in _comp_marks.values() if v == 'A')} Absent)."
                    )
                except Exception as _ce:
                    st.error(f"Competition export failed: {_ce}")
        else:
            st.warning(
                "⚠️ `attendance_manager` is not available. "
                "Install it to enable the Competition Format export."
            )
